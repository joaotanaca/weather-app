...
const WeatherCard = observer(({ id, name, temperature, className, weatherCode, isDay }: TWeatherCard) => {
  ...
  return (
    ...
    >
      <header className="w-full flex items-center justify-between text-lg font-semibold">
        {name}
        <Dropdown
          className="min-w-fit px-3 translate-x-2 border-none"
          options={[
            {
      ...
      <footer className="flex justify-center text-2xl font-normal">{temperature}</footer>
    </article>
  );
});
