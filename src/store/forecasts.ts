...
class Forecasts {
    ...
    constructor() {
        ...
        this.cityStore = city;
        const localStorageForecast = secureLocalStorage.get(
            "forecasts",
        ) as string;

        if (localStorageForecast)
            this.forecasts = JSON.parse(localStorageForecast);
    }

    addForecast = async () => {
        const { data } = await forecast.today(
            this.cityStore.citySelected.lat,
            this.cityStore.citySelected.lon,
        );
    ...
    updateForecastCity = async (id: string) => {
        const index = this.forecasts.findIndex(
            ({ id: identifier }) => identifier === id,
        );
